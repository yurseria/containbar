...
fn on_tray_click(this: &TrayTarget, button: MouseButton) {
    ...
    unsafe {
        ...
        if button == MouseButton::Right || (menu_on_left_click && button == MouseButton::Left) {
            // Keep the menu out of the RefCell while performClick runs its
            // nested event loop; set_menu may be called re-entrantly there.
            let menu = this.ivars().menu.borrow().clone();
            let has_items = menu.as_ref().is_some_and(|menu| menu.numberOfItems() > 0);
            if has_items {
                // On macOS 27, a permanently attached menu consumes clicks
                // before TrayTarget receives them. Attach it only while open.
